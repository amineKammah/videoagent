...
def test_normalize_blob_path_accepts_bucket_uri(monkeypatch: pytest.MonkeyPatch):
    ...
    client = storage_module.GCSStorageClient(bucket_name="videoagent-assets")
    ...
    assert client.to_gs_uri("companies/acme/videos/a.mp4") == "gs://videoagent-assets/companies/acme/videos/a.mp4"
...
def test_get_storage_client_caches_and_rebuilds_on_env_change(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("GCS_BUCKET_NAME", "videoagent-assets")
    ...
    assert created == [("videoagent-assets", 900, "europe-west2")]
    ...
    assert created[-1] == ("videoagent-assets", 600, "europe-west2")
